...
const Player = () => {
  let formattedAudioDuration;
  function duration(d) {
    const audioDuration = moment.duration(d, "seconds");
    const hours = Math.floor(audioDuration.asHours());
    if (hours > 0) {
      formattedAudioDuration = moment
        .utc(audioDuration.asMilliseconds())
        .format("HH:mm:ss");
    } else {
      // Format as "mm:ss"
      formattedAudioDuration = moment
  ...
  const quranChapters = [
    ...
    { name: "Al-Ala", number: 87 },
    { name: "Al-Ghashiya", number: 88 },
    ...
    { name: "Az-Zalzala", number: 99 },
    { name: "Al-Adiyat", number: 100 },
    { name: "Al-Qaria", number: 101 },
    { name: "At-Takathur", number: 102 },
    { name: "Al-Asr", number: 103 },
    { name: "Al-Humaza", number: 104 },
  ...
  const aud = useRef();
  const elem_name = useRef();
  const elem_time = useRef();
  let isPlay = false;
  let isLoop = false;
  let isShuffle = false;
  let num = 0;
  let surah = [];
  function load() {
    aud.current.src = `https://download.quranicaudio.com/qdc/mishari_al_afasy/murattal/${
      num + 1
    }.mp3`;
  ...
  function shuffle() {
    isShuffle = true;
    num = Math.floor(Math.random() * 114);
    load();
    play();
  }
  ...
  function next() {
    num++;
    num > 114 - 1 ? (num = 0) : (num = num);
    load();
    play();
  }
  function prev() {
    num--;
    num < 0 ? (num = 114 - 1) : (num = num);
    load();
    play();
  }
  function loop() {
  ...
  useEffect(() => {
    load();
    console.log("iam rendring");
  }, []);

  return (
    <>
      <div className="cont">
        <div ref={elem_name} className="element_data">
          Name OF Surah
        </div>
        <div ref={elem_time} className="time">
          00:00
        </div>
